<template>
  <filter-component v-if="!reservationId && !personId && !casketId && !urnId" :filters="filters" @update-filters="sendFilters" />
  ...
  setup(props) {
    ...
    const fetchDepositData = async (page = 1) => {
      try {
        ...
        let data = null
        if (props.urnId) {
          data = await depositStore.getDepositByUrnId(`${props.urnId}?includePerson=true&includeCasket=true&includeReservation=true`)
